...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title,
    description: getPrivacy(locale).intro.slice(0, 160),
    alternates: {
      canonical: `/${locale}/privacidad/`,
      languages: {
        ...Object.fromEntries(LOCALES.map((l) => [l, `/${l}/privacidad/`])),
...
export default async function PrivacyPage({ params }: Props) {
  ...
  const breadcrumb = buildBreadcrumb([
    ...
    { name: dict.legal.privacyTitle, path: `/${locale}/privacidad/` },
  ]);

  return (
    <>
      <JsonLd data={breadcrumb} />
      <LegalArticle title={dict.legal.privacyTitle} doc={doc} locale={locale} dict={dict} />
    </>
  );
}
