...
export default async function ExperienceDetailPage({ params }: DetailPageProps) {
  ...
  return (
    <>
      {/* Breadcrumb */}
      <nav className="bg-(--color-brand-cream-100)/60 border-b border-(--color-border)">
      ...
      <section className="bg-(--color-brand-green-800) text-(--color-brand-cream-100)">
        <div className="mx-auto grid max-w-6xl gap-8 px-6 py-12 md:grid-cols-[1fr_1.3fr] md:items-center md:py-16">
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-(--color-brand-orange-400)">
              {verticalLabel} · {exp.category}
            </p>
            <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">{title}</h1>
            {shortDesc && (
              <p className="mt-5 text-lg text-(--color-brand-cream-300)">{shortDesc}</p>
            )}
            <div className="mt-6 flex flex-wrap gap-2 text-xs">
              {duration && (
                <span className="rounded-full bg-(--color-brand-cream-100)/10 px-3 py-1">
                  {duration}
                </span>
              )}
              <span className="rounded-full bg-(--color-brand-cream-100)/10 px-3 py-1 capitalize">
                {difficulty}
              </span>
              <span className="rounded-full bg-(--color-brand-cream-100)/10 px-3 py-1 capitalize">
                {exp.city.replace(/-/g, " ")}
              ...
              {rating && (
                <span className="inline-flex items-center gap-1 rounded-full bg-(--color-brand-orange-500)/15 px-3 py-1 text-(--color-brand-cream-100)">
                  <span className="text-(--color-brand-orange-400)" aria-hidden>★</span>
                  {rating.label}
                </span>
              )}
            </div>
          </div>
          <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl bg-(--color-brand-green-700) shadow-lg">
            {cover ? (
              <Image
                src={cover}
                alt={title}
              ...
              <div className="flex h-full w-full items-center justify-center text-(--color-brand-cream-300)/40">
                NorteWalk
              </div>
            )}
          </div>
              ...
              {extraImages.map((img, idx) => {
                ...
                return (
                  <div
                    ...
                    className="relative aspect-square overflow-hidden rounded-xl bg-(--color-brand-green-100)"
                  >
                    {url && (
                      <Image
                        src={url}
      ...
      <section className="bg-(--color-background)">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-12 md:grid-cols-[1.6fr_1fr] md:py-16">
          ...
          <div className="space-y-12">
            {longDesc && (
              <article>
                <h2 className="mb-4 text-2xl font-semibold text-(--color-foreground)">
            ...
            {exp.itinerary && exp.itinerary.length > 0 && (
              <article>
                <h2 className="mb-6 text-2xl font-semibold text-(--color-foreground)">
                  {dict.detail.itinerary}
                </h2>
                <ol className="relative space-y-6 border-l-2 border-(--color-brand-green-200) pl-6">
                  {[...exp.itinerary]
                    .sort((a, b) => Number(a.step_order) - Number(b.step_order))
                    .map((step) => (
                      <li key={step.id ?? step.step_order} className="relative">
                        <span className="absolute -left-[33px] flex h-6 w-6 items-center justify-center rounded-full bg-(--color-brand-green-600) text-xs font-bold text-(--color-brand-cream-100)">
                        ...
                        {step.duration_min !== null && step.duration_min !== undefined && (
                          <p className="mt-0.5 text-xs uppercase tracking-wider text-(--color-brand-orange-500)">
            ...
            {meetingPoint && (
              <article>
                <h2 className="mb-3 text-2xl font-semibold text-(--color-foreground)">
                ...
                {mapsUrl && (
                  <a
                    ...
                    className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-(--color-brand-orange-600) hover:text-(--color-brand-orange-700) hover:underline"
                  >
            ...
            <BookingPanel
              ...
              recap={
                <div className="flex h-full flex-col gap-5">
                  {cover && (
                    <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-(--color-brand-green-700)">
                      <Image
                        src={cover}
                        alt={title}
                        fill
                  ...
                  <div>
                    ...
                    <h3 className="mt-1 text-xl font-semibold leading-tight text-(--color-brand-cream-100)">
                      {title}
                    </h3>
                  </div>
                  <div className="rounded-xl bg-(--color-brand-green-900)/40 p-4">
                    <p className="text-2xl font-bold">{price.label}</p>
                    {price.hint && (
                      <p className="text-xs text-(--color-brand-cream-300)">{price.hint}</p>
                    )}
                  </div>
                  <dl className="space-y-2.5 text-sm text-(--color-brand-cream-300)">
                    {duration && (
                  ...
                  {meetingPoint && (
                    <div className="border-t border-(--color-brand-cream-100)/15 pt-4">
                      <p className="text-xs uppercase tracking-widest text-(--color-brand-orange-400)">
                        {dict.detail.meetingPoint}
                      </p>
                      <p className="mt-1 text-sm text-(--color-brand-cream-100)">
                        {meetingPoint}
                      </p>
                      {mapsUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="mt-1 inline-block text-xs font-medium text-(--color-brand-orange-400) hover:text-(--color-brand-orange-300) hover:underline"
                        >
                          {dict.detail.viewOnMap} →
                        </a>
                      )}
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-auto inline-flex items-center justify-center gap-1 rounded-full border border-(--color-brand-cream-100)/30 px-4 py-2 text-xs font-medium text-(--color-brand-cream-100) transition-colors hover:bg-(--color-brand-cream-100)/10"
                  >
...
function RecapRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      <dt className="text-(--color-brand-cream-300)/70">{label}</dt>
      <dd className="font-semibold text-(--color-brand-cream-100) text-right">{value}</dd>
    </div>
  );
}
...
function InclusionList({
  ...
  const iconColor =
    variant === "included" ? "text-(--color-brand-green-600)" : "text-(--color-brand-orange-700)";
  return (
    <div>
