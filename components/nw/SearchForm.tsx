...
export default function SearchForm({ locale, dict, cities }: SearchFormProps) {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="
        grid gap-0 overflow-hidden rounded-2xl bg-(--color-bone-100) shadow-[0_30px_80px_-20px_rgba(0,0,0,0.55)]
        sm:grid-cols-[1.1fr_1fr_1.1fr_auto] sm:items-stretch
      "
    >
      {/* Destino */}
      <label className="group flex flex-col gap-1 px-5 py-4 border-b border-(--color-stone-700)/12 sm:border-b-0 sm:border-r">
        <span className="font-(family-name:--font-mono) text-[10px] uppercase tracking-[0.18em] text-(--color-stone-500)">
          {dict.search.destinationLabel}
        </span>
        <select
          value={city}
          onChange={(e) => setCity(e.target.value)}
          className="bg-transparent text-[14.5px] font-medium text-(--color-stone-800) outline-none"
        >
      ...
      <button
        ...
        disabled={!vertical}
        className="
          flex items-center justify-center gap-2 bg-(--color-accent-500) px-7 py-4
